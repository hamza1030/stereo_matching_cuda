...
void compute_guided_filter(unsigned char* i1, unsigned char* i2, float* cost, float* filtered, unsigned char* mean1, unsigned char* mean2, int w1, int h1, int w2, int h2, int size_d, bool host_gpu_compare) {
	...
	float* integral_im1CPU = (float*)malloc(n1_fl);
	memset(integral_im1CPU, 0.0f, n1_fl);

	integral(h_im1, integral_im1, w1, h1);
	integralOnCPU(h_im1, integral_im1CPU, w1, h1);

	//check_errors(integral_im1, integral_im1CPU, w1 * h1);
	for (int i = 0; i < w1; i++) {
		for (int j = 0; j < h1; j++)
		{
			if (integral_im1CPU[i + w1 * j] != integral_im1[j + h1 * i]) {
				//res = false;
				cout << "error at element: " << i << " ResultGPU = " << integral_im1[j + h1 * i] << " and ResultCPU= " << integral_im1CPU[i + w1 * j] << endl;
			}
		}
	}
